  ...
  <div>
    ...
    >
      ...
      <span v-if="expiresIn">
        ...
        <span v-if="expiresIn > 86400e3">
          {{ (expiresIn / 86400e3).toFixed(0) }} days.
        </span>
  ...
  async asyncData({ $axios, params, res, error }) {
    try {
      ...
      return {
        ...
        id: params.id,
        expiresIn:
          entry.preview && entry.preview.exp
            ? entry.preview.exp * 1000 - Date.now()
            : undefined,
      }
    } catch (e) {
      console.error(e)
  ...
  computed: {
    ...
    syndication() {
      return [
        ...
        {
          url: this.entry.frontmatter.reddit,
          title: 'Reddit',
